...
function renderDrawerPlaylist() {
  ...
  gothDrawerList.replaceChildren();
  ...
  playlist.forEach((track, i) => {
    ...
    number.textContent = i === currentIndex && !audio.paused ? "▶" : `#${i + 1}`;
...
function renderMusicTabPlaylist() {
  ...
  playlist.forEach((track, i) => {
    ...
    artist.className = "mr-artist";
    artist.title = cleanArtist;
    artist.textContent = cleanArtist;

    row.append(number, title, artist);
...
const MAX_LOCAL_AUDIO_SIZE = 100 * 1024 * 1024;

function handleCustomAudioFile(file) {
  if (!file) return;

  const hasAudioMime = !file.type || file.type.startsWith("audio/");
  if (!hasAudioMime || file.size > MAX_LOCAL_AUDIO_SIZE) {
    if (gothHudStatus) gothHudStatus.textContent = "INVALID FILE";
    return;
  }
  ...
  const cleanName = file.name.replace(/\.[^/.]+$/, "").slice(0, 120).toUpperCase();

  audio.src = customObjectUrl;
...
async function initAboutYou() {
  ...
  // Fetch Public IP without cookies/referrer and with a short timeout.
  ...
  try {
    ...
    if (ipEl && ip.length <= 45 && /^[0-9a-f:.]+$/i.test(ip)) {
      ipEl.textContent = ip;
    }
  } catch {
    if (ipEl) ipEl.textContent = "0.0.0.0";
  } finally {
